...
class AppStore extends Component {
  state = {tabCategory: tabsList[0].tabId, input: ''}

  changeCategory = category => {
    this.setState({tabCategory: category})
  }
  ...
  filterAppsList = (selectedCategory, input) => {
    const filteredList = appsList.filter(
      eachItem => eachItem.category === selectedCategory,
    )
    const reFilteredList = filteredList.filter(eachItem =>
  ...
  render() {
    const {tabCategory, input} = this.state
    const filteredList = this.filterAppsList(tabCategory, input)
    ...
    return (
      <div className="main-container">
        ...
        <div className="search-container">
          ...
          />
          <img src={SEARCH_ICON_URL} alt="search icon" className="search" />
        </div>
